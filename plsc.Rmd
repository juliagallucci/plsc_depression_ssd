...
res.pls.plot$results.graphs$ctrY.plot + geom_errorbar(aes(ymin = lv3_ci[,1], ymax = lv3_ci[,2])) + ylim(-60,20)
```
...
rs_conn_LV3[rs_conn_LV3$rois %in% names(which(rs_plsc_boot_regress$bootRatiosSignificant.i[,3] == TRUE)), "r"] <- 10000*(Cx_conn[rownames(Cx_conn) %in% names(which(rs_plsc_boot_regress$bootRatiosSignificant.i[,3] == TRUE)),1])/12
...
# change r values to contributions for significant connections - the contributions are very small values, hence the x10000 for visualization
# divided by 12 to limit the range between 0 and 1 for plot alpha - check range()
rs_conn_LV2[rs_conn_LV2$rois %in% names(which(rs_plsc_boot_regress$bootRatiosSignificant.i[,2] == TRUE)), "r"] <- 10000*(Cx_conn[rownames(Cx_conn) %in% names(which(rs_plsc_boot_regress$bootRatiosSignificant.i[,2] == TRUE)),1])/12
# make all other connections 0
...
con_lv1_pos <- cbind(X_non0[,"R_31a_ROI-L_31pd_ROI"], data_Y_non0_fa$MR1)
con_lv1_pos <- as.data.frame(con_lv1_pos)
colnames(con_lv1_pos) <-c('R_31a_ROI__L_31pd_ROI','Factor1')
plot_con_lv1_pos <- ggplot(con_lv1_pos,aes(y=R_31a_ROI__L_31pd_ROI,x=Factor1,))+geom_point()+stat_smooth(method="lm",se=TRUE) +xlab('General Depression Factor')+ ylab('Connectivity') +theme_bw() +  theme(axis.text.x=element_text(size=12),axis.text.y=element_text(size=12))
plot_con_lv1_pos
cor(con_lv1_pos)
...
con_lv1_neg <- cbind(X_non0[,"R_LO2_ROI-R_V3CD_ROI"], data_Y_non0_fa$MR1)
con_lv1_neg <- as.data.frame(con_lv1_neg)
colnames(con_lv1_neg) <-c('R_LO2_ROI__R_V3CD_ROI','Factor1')
plot_con_lv1_neg <- ggplot(con_lv1_neg,aes(y=R_LO2_ROI__R_V3CD_ROI,x=Factor1,))+geom_point()+stat_smooth(method="lm",se=TRUE) +xlab('General Depression Factor')+ ylab('Connectivity') +theme_bw() +  theme(axis.text.x=element_text(size=12),axis.text.y=element_text(size=12))
plot_con_lv1_neg
cor(con_lv1_neg)
```
...
#scatterplot R_31pv_ROI-R_9.46d_ROI
con_lv2_pos <- cbind(X_non0[,"R_31pv_ROI-R_9.46d_ROI"], Y_non0$sans_sub_asocial_anhedonia)
con_lv2_pos <- as.data.frame(con_lv2_pos)
colnames(con_lv2_pos) <-c('R_31pv_ROI__R_9.46d_ROI','SANS_Anhedonia')
plot_con_lv2_pos <- ggplot(con_lv2_pos,aes(y=R_31pv_ROI__R_9.46d_ROI,x=SANS_Anhedonia,))+geom_point()+stat_smooth(method="lm",se=TRUE) +xlab('SANS Anhedonia')+ ylab('Connectivity') +theme_bw() +  theme(axis.text.x=element_text(size=12),axis.text.y=element_text(size=12))
plot_con_lv2_pos
cor(con_lv2_pos)
```
...
#scatterplot R_p32pr_ROI-L_4_ROI
con_lv2_neg <- cbind(X_non0[,"R_p32pr_ROI-L_4_ROI"], Y_non0$sans_sub_asocial_anhedonia)
con_lv2_neg <- as.data.frame(con_lv2_neg)
colnames(con_lv2_neg) <-c('R_p32pr_ROI__L_4_ROI','SANS_Anhedonia')
plot_con_lv2_neg <- ggplot(con_lv2_neg,aes(y=R_p32pr_ROI__L_4_ROI,x=SANS_Anhedonia,))+geom_point()+stat_smooth(method="lm",se=TRUE) +xlab('SANS Anhedonia')+ ylab('Connectivity') +theme_bw() +  theme(axis.text.x=element_text(size=12),axis.text.y=element_text(size=12))
plot_con_lv2_neg
cor(con_lv2_neg)
```
...
#scatterplot R_46_ROI-L_9p_ROI
con_lv3_pos <- cbind(X_non0[,"R_46_ROI-L_9p_ROI"], data_Y_non0_fa$MR2)
con_lv3_pos <- as.data.frame(con_lv3_pos)
colnames(con_lv3_pos) <-c('R_46_ROI__L_9p_ROI','Factor2')
plot_con_lv3_pos <- ggplot(con_lv3_pos,aes(y=R_46_ROI__L_9p_ROI,x=Factor2,))+geom_point()+stat_smooth(method="lm",se=TRUE) +xlab('Guilt Factor')+ ylab('Connectivity') +theme_bw() +  theme(axis.text.x=element_text(size=12),axis.text.y=element_text(size=12))
plot_con_lv3_pos
cor(con_lv3_pos)
```
```{r fig.height=1.5, fig.width=1.25}
...
#scatterplot 
con_lv3_neg <- cbind(X_non0[,"THA.VA.rh-R_24dv_ROI"], data_Y_non0_fa$MR2)
con_lv3_neg <- as.data.frame(con_lv3_neg)
colnames(con_lv3_neg) <-c('THA.VA.rh__R_24dv_ROI','Factor2')
plot_con_lv3_neg <- ggplot(con_lv3_neg,aes(y=THA.VA.rh__R_24dv_ROI,x=Factor2,))+geom_point()+stat_smooth(method="lm",se=TRUE) +xlab('Guilt Factor')+ ylab('Connectivity') +theme_bw() +  theme(axis.text.x=element_text(size=12),axis.text.y=element_text(size=12))
plot_con_lv3_neg
cor(con_lv3_neg)
```
...
for (y in Loadings_LV2$y){
  cat(rois[Glasser_Tian_roi_info$atlas_roi == y,]$net_ColeAnt, '\n')
}
...
lv2_neg_networks = unique(Loadings_LV2_neg$x_y_network)
lv2_pos_networks = unique(Loadings_LV2_pos$x_y_network)

clv2_df_neg = data.frame(matrix(ncol = 2))
colnames(clv2_df_neg) = c("network", "sum_net")
for (network in lv2_neg_networks){
  sum_net = sum(Loadings_LV2_neg[Loadings_LV2_neg$x_y_network == network,]$r)
...
for (network in lv2_pos_networks){
  sum_net = sum(Loadings_LV2_pos[Loadings_LV2_pos$x_y_network == network,]$r)
...
lv3_neg_networks = unique(Loadings_LV3_neg$x_y_network)
lv3_pos_networks = unique(Loadings_LV3_pos$x_y_network)

clv3_df_neg = data.frame(matrix(ncol = 2))
colnames(clv3_df_neg) = c("network", "sum_net")
for (network in lv3_neg_networks){
  sum_net = sum(Loadings_LV3_neg[Loadings_LV3_neg$x_y_network == network,]$r)
...
for (network in lv3_pos_networks){
  sum_net = sum(Loadings_LV3_pos[Loadings_LV3_pos$x_y_network == network,]$r)
